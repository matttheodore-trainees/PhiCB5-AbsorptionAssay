...
After normalization with respect to the control genotype 'bNY30a wt', summary statistics were computed. These include the mean, standard deviation, and standard error for each genotype and incubation time, grouped by date/genotype. 

#### Control Data Extraction and Normalization

The control data ('bNY30a wt') were separated, and their average, standard deviation, and standard error were calculated. This control data is critical for normalizing the other genotypes. Subsequently, error propagation was performed to calculate the normalized average along with the propagated standard deviation and standard error for each genotype. 

#### ANOVA and Dunnett's Test

A one-way Analysis of Variance (ANOVA) was performed to test the hypothesis that genotype significantly influences the normalized average titer. The F-statistic and p-value from the ANOVA were calculated, and a post-hoc Dunnett's test was performed to compare each genotype against the control 'bNY30a wt'. The Dunnett's test specifically controls the family-wise error rate while making multiple comparisons to a single control group.

#### Plotting

Finally, a bar plot was generated to visualize the normalized average titer for each genotype, with error bars representing the standard error. Asterisks above the bars denote genotypes that are significantly different from the control, as determined by Dunnett's test.

### Key Results and Tables

#### ANOVA Table

The ANOVA table suggests that the genotype has a significant effect on the normalized average titer. The F-statistic and p-value confirm this, warranting further post-hoc tests for pairwise comparisons.

| Source   | Sum of Squares  | Degrees of Freedom | F-Value  | p-value  |
|----------|-----------------|--------------------|----------|----------|
| Genotype | 1.853           | 7                  | 4.5341   | 0.000455 |

#### Dunnett's Test Results

The Dunnett's test was used for post-hoc comparisons between each genotype and the control 'bNY30a wt'. The T-Statistic and P-Value for each genotype are reported in the following table.

```plaintext
  Genotype  T-Statistic   P-Value
0   mutant1    -0.554510  0.581865
1   mutant2     1.636995  0.110783
2   mutant3    -2.510739  0.016205
3   mutant4    -1.944234  0.058957
4   mutant5    -0.422278  0.675378
5   mutant6     3.227123  0.003053
6   mutant7    -1.107388  0.273642
```

#### Plot

The bar plot, enriched with individual data points and error bars, provides a visual representation of the normalized average titer across genotypes. Asterisks denote genotypes that significantly differ from the control 'bNY30a wt'.

![Normalized Phage Adsorption by Genotype](data:image/png;base64,...)

#### Summary

In our investigation of the role of genotype in shaping the outcomes of the assay, we employed a one-way Analysis of Variance (ANOVA) to test the hypothesis that genotype significantly influences the normalized average of the measured variable. The ANOVA model revealed a highly significant effect of genotype on the assay outcomes (\(F(7, 224) = 4.5341\), \(p < 0.0005\)). This compelling statistical evidence underscores the biological relevance of genotype in modulating the assay metrics and establishes a basis for further mechanistic investigations. These findings are integral for understanding the genotype-specific variations and offer a robust statistical foundation for downstream analyses.
...
# Load the 'data_complete.xlsx' dataset
file_path_complete = 'Data/data_complete.xlsx'
data_complete = pd.read_excel(file_path_complete)
...
```{python}
# Data Normalization
# Merge the "bNY30a wt" data with the main data to normalize
merged_data = pd.merge(data_complete, 
                       data_complete[data_complete['genotype'] == 'bNY30a wt'][['date', 'titer']], 
                       on='date', 
                       suffixes=('', '_control'))
...
summary_stats = merged_data.groupby(['date', 'genotype']).agg(
...
control_data = summary_stats[summary_stats['genotype'] == 'bNY30a wt'].copy()
